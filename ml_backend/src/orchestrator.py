                ...
                'stages': {
                    ...
                    'recommendations': {
                        ...
                        'items': [  # Convert Recommendation objects to dicts
                            {
                                'title': str(rec.title if hasattr(rec, 'title') else 'Unknown'),
                                'description': str(rec.description if hasattr(rec, 'description') else ''),
                                'estimated_impact': float(
                                    rec.impact_prediction.expected_improvement if hasattr(rec, 'impact_prediction') 
                                    else getattr(rec, 'estimated_impact', 0)
                                ),
                                'cost_usd': float(
                                    rec.cost_estimate.get_total_usd() if hasattr(rec, 'cost_estimate') and hasattr(rec.cost_estimate, 'get_total_usd')
                                    else getattr(rec, 'cost_usd', 0)
                                ),
                                'priority': str(rec.priority.value if hasattr(rec, 'priority') and hasattr(rec.priority, 'value') else str(rec.priority) if hasattr(rec, 'priority') else 'medium'),
                                'category': str(rec.category.value if hasattr(rec, 'category') and hasattr(rec.category, 'value') else str(rec.category) if hasattr(rec, 'category') else 'other')
                            } if hasattr(rec, 'title') else str(rec)
...
class SynthosOrchestrator:
    ...
    async def validate(
        ...
        try:
            # Wrap data loading with retry and timeout
            @with_retries(max_attempts=3, initial_wait=2.0)
            async def load_with_retry():
                return await with_timeout(
                    self.dataset_loader.load_dataset(dataset_path, dataset_format),
                    timeout_seconds=300.0,  # 5 minute timeout
                    operation_name="Data Loading"
                )
            
            dataset = await load_with_retry()
            ...
            if reference_dataset_path:
                try:
                    ...
                    async def load_reference():
                        return await self.dataset_loader.load_dataset(
                            reference_dataset_path,
                            dataset_format
                        )
                    reference_data = await load_reference()
                except Exception as e:
                    logger.warning(f"Failed to load reference dataset, will use self-comparison: {e}")
                    reference_data = None
            ...
            # Don't retry validation errors (bad input)
            logger.error(f"❌ Data validation failed: {e}")
            raise
        except ResourceExhaustedError as e:
            # Resource issues (memory, disk)
            logger.error(f"❌ Resource exhausted during data loading: {e}")
            raise
        except Exception as e:
            error_category = classify_error(e)
            logger.error(f"❌ Data loading failed ({error_category.value}): {e}")
            raise
        ...
        try:
            # Wrap diversity analysis with timeout
            diversity_result = await with_timeout(
                self.diversity_analyzer.analyze_diversity(dataset_path, dataset_format),
                timeout_seconds=600.0,  # 10 minute timeout
                operation_name="Diversity Analysis"
            )
        except Exception as e:
            # Use graceful degradation - provide default diversity score
            logger.warning(f"Diversity analysis failed, using default score: {e}")
            diversity_result = type('obj', (object,), {
                'overall_score': 50.0,
                'dimension_scores': {}
            })
